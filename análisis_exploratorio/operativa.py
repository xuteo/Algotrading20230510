...
class MT5:
    ...
    def get_data_multi_index(symbol, n, timeframe=mt5.TIMEFRAME_D1):
         """ Función para importar los datos del símbolo elegido
         con los times index segmentados """
         ...
         # Set column time as the index of the dataframe    
         rates_frame['date'] =  pd.DatetimeIndex(rates_frame['time']).date
         rates_frame['year'] = pd.DatetimeIndex(rates_frame['time']).year
         rates_frame['month'] = pd.DatetimeIndex(rates_frame['time']).month
         rates_frame['day'] = pd.DatetimeIndex(rates_frame['time']).day
         rates_frame['hour'] = pd.DatetimeIndex(rates_frame['time']).hour
         rates_frame['minute'] = pd.DatetimeIndex(rates_frame['time']).minute
         rates_frame['secods'] = pd.DatetimeIndex(rates_frame['time']).second
         rates_frame['miliseconds'] = pd.DatetimeIndex(rates_frame['time']).microsecond
         rates_frame = rates_frame.set_index('time') # Reemplaza index automatico
         rates_frame = rates_frame.set_index(['date','year', 'month', 'day', 'hour', 'minute', 'secods', 'miliseconds'], append=True)
